...
def render():
    ...
    with left_col:
        ...
        <p style="color:rgba(255,255,255,0.7) !important; font-size:15px; max-width:400px;">Access your owner dashboard and AI insights. Monitor occupancy, revenue, and court health in real-time.</p>
        """, unsafe_allow_html=True)

        st.markdown("""
        <div style="display:flex; gap:1rem; margin-top:2rem;">
            <div style="padding:12px 16px; background:rgba(255,255,255,0.1); backdrop-filter:blur(12px); border-radius:16px;">
                <span style="font-family:'Lexend'; font-size:9px; text-transform:uppercase; letter-spacing:0.1em; color:rgba(255,255,255,0.6);">LIVE TRAFFIC</span><br>
                <span style="font-family:'Space Grotesk'; font-weight:700; font-size:1.5rem; color:white;">84%</span>
            </div>
            <div style="padding:12px 16px; background:rgba(255,255,255,0.1); backdrop-filter:blur(12px); border-radius:16px;">
                <span style="font-family:'Lexend'; font-size:9px; text-transform:uppercase; letter-spacing:0.1em; color:rgba(255,255,255,0.6);">REV</span><br>
    ...
    with right_col:
        ...
        <div class="glass-card" style="margin-top:3rem;">
            ...
            <p style="color:#535b71 !important; font-size:13px; margin-bottom:1.5rem;">Enter your credentials to manage your venue ecosystem.</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("Log In to Dashboard →", type="primary", use_container_width=True, key="owner_login_btn"):
            st.session_state.logged_in = True
            st.session_state.flow = "owner"
            navigate("owner_dashboard")

        st.markdown('<div style="text-align:center; margin:1rem 0; font-size:12px; color:#535b71;">OR AUTHENTICATE WITH</div>', unsafe_allow_html=True)

        if st.button("🏢 Corporate ID", use_container_width=True, key="corp_id_login"):
            st.session_state.logged_in = True
            st.session_state.flow = "owner"
            navigate("owner_dashboard")

        st.markdown('<div style="text-align:center; margin-top:1.5rem; font-size:13px; color:#535b71;">New operator? <strong style="color:#506300; cursor:pointer;">Manage your venues</strong></div>', unsafe_allow_html=True)
